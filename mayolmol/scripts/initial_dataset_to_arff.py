...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        ...
        if op.exists(dataset):
            ...
            label = sys.argv[2]
            prepared_data, master_file = prep.prepare_user_dataset(directory, file, label)
            ...
            arff.spectrophores_to_arff(directory, op.basename(master_file), op.basename(spectrophores_file), label)
            print "Converting to .arff format the intrinsic descriptors file %s."%descriptors_file
            arff.cdk_desc_to_arff(directory, op.basename(master_file), op.basename(descriptors_file), label)
            for file in fingerprint_files:
                print "Converting to .arff format the fingerprint file %s."%file[1]
                arff.cdk_fpt_to_arff(directory, op.basename(master_file), op.basename(file[1]), label, file[0])
            ...
            mlio.merge_arff2(directory, dest_arff_master, dest_arff3, op.splitext(prepared_data)[0] + "-cdk-extended.arff", False)
        else:
